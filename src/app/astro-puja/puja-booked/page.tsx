...
const PujaBooked: React.FC = () => {
  ...
  const columns = [
    ...
    { 
      name: 'Order Id', 
      selector: (row: PujaBookedItem) => row?.paymentDetails?.razorpayOrderId || '',
      width: '190px'
    },
    { 
      name: 'Customer', 
      selector: (row: PujaBookedItem) => row?.customerId?.customerName || '' 
    },
    { 
      name: 'Contact Number', 
      selector: (row: PujaBookedItem) => row?.paymentDetails?.contact || ''
    },
    {
