...
import { Loader2, Clock, Trash2, AlertCircle, CalendarOff } from 'lucide-react';
...
interface AstrologerUnavailabilityProps {
  astrologerId: string;
  astrologerName: string;
}

export default function AstrologerUnavailability({ astrologerId, astrologerName }: AstrologerUnavailabilityProps) {
  ...
  const handleBlockTimeRange = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(data.message || "Unavailability marked successfully");
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-red-500 to-red-600 px-6 py-4 rounded-lg shadow-md">
        <h2 className="text-xl font-bold text-white flex items-center">
          <CalendarOff className="w-6 h-6 mr-3" />
          Manage Unavailability
        </h2>
        <p className="text-red-100 mt-1 text-sm">Block time slots when you're not available for consultations</p>
      </div>

      <Card className="p-6 border-gray-200 bg-white shadow-md">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Mark Unavailability</h3>
      ...
      <Card className="p-6 border-gray-200 bg-white shadow-md">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900">
            Your Unavailability for {formatDate(selectedDate)}
          </h3>
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-gray-600">
              You haven't marked any unavailability for this date
            </p>
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-700">
                      <span className="font-medium">Report Type:</span> {block.prefix || 'All Reports'}
                    </p>
                  </div>
