...
const getGiftData = async () => {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/admin/get-all-gift`
    );
    const data: ApiResponse<Gift[]> = await response.json();

    if (data.success) {
      // Sort descending by createdAt (latest first)
      const sortedGifts = (data.gift || []).sort(
        (a: Gift, b: Gift) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      );
      ...
      cell: (row: Gift) => (
        <div className="flex justify-center">
          <div className="relative w-12 h-12">
            <img
              src={row?.giftIcon ? `${process.env.NEXT_PUBLIC_IMAGE_URL3}${row.giftIcon}` : '/images/placeholder.png'}
              alt="Gift icon"
