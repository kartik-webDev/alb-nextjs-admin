 ...
 const getAstrologersReviews = async () => {
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/admin/get-all-review`
    );
  ...
  const columns = [
    ...
    { 
      name: 'Customer', 
      selector: (row: Review) => row?.customerName || ''
    },
    { 
      name: 'Astrologer', 
      selector: (row: Review) => row?.astrologerName || ''
    },
    { 
      name: 'Rating', 
      selector: (row: Review) => (
    ...
    { 
      name: 'Service', 
      selector: (row: Review) => (
        <div className="capitalize">{row?.type?.toLowerCase() || ''}</div>
      )
    },
    { 
      name: 'Comment', 
