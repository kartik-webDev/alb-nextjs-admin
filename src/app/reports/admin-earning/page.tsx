...
const getAstrologerName = (astrologerId: string | null | AstrologerDetails): string => {
  ...
  if (typeof astrologerId === 'object' && astrologerId !== null) {
    return (astrologerId as AstrologerDetails).astrologerName || '';
  }
  return '';
};
...
const AdminEarning: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row: AdminEarningRow) => {
        ...
        if (typeof row.astrologerId === 'object' && row.astrologerId !== null) {
          return (row.astrologerId as AstrologerDetails)._id || '';
        }
        return row.astrologerId || '';
      },
      omit: true, 
      export: true,
    },
    ...
    {
      name: 'Customer ID',
      selector: (row: AdminEarningRow) => row?.customerId?._id || '',
      omit: true, 
      export: true, 
    },
    {
      name: 'Customer Name',
      selector: (row: AdminEarningRow) => row?.customerId?.customerName || '',
      width: '140px',
      export: true,
    },
    {
    ...
    },
    {
      ...
      cell: (row: AdminEarningRow) => (row?.duration ? `${row.duration} min` : ''),
      export: true,
    ...
    },
    {
      ...
      cell: (row: AdminEarningRow) => row?.startTime || '',
      export: true,
    },
    {
      name: 'End Time',
      selector: (row: AdminEarningRow) => row?.endTime || '',
      cell: (row: AdminEarningRow) => row?.endTime || '',
      export: true,
    },
    {
      name: 'Date',
    ...
    {
      name: 'Transaction ID',
      selector: (row: AdminEarningRow) => row?.transactionId || '',
